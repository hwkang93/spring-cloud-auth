...
import org.geopaas.auth.security.Aes256Util;
 ...
 * JWT 관리 클래스
 *
 *  JWT 를 발급하고, 검증하는 클래스. application.yml 에 작성된 만료시간, API 키 등을 기준으로 한다.
 *
 */
@Slf4j
@Component
...
public class JwtTokenProvider implements InitializingBean {
    ...
    public Jwt generate(Authentication authentication) {
        long accessTokenExpireTime = 1000 * 60 * jwtProperties.getAccessTokenExpiration();
        long refreshTokenExpireTime = 1000 * 60 * 60 * 24 * jwtProperties.getRefreshTokenExpiration();
        ...
        String accessToken = Jwts.builder()
                ...
                .claim(AUTH, Aes256Util.encode(authorities))                // payload "auth": "tmi~~~"
    ...
    public Authentication getAuthentication(String accessToken) {
        ...
        String auth = Aes256Util.decode(claims.get(AUTH).toString());
        // 클레임에서 권한 정보 가져오기
